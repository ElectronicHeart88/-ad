...
def veheoanimationchapcanhbayxunduoi(screen, x, y, scale):
    ...
    left_wing = [
        (int(x - 120*scale), int(y + 50*scale)), # Hạ thấp
        (int(x - 320*scale), int(y + 10*scale)),  # Dẹt hơn
        (int(x - 250*scale), int(y + 40*scale)),   
        (int(x - 350*scale), int(y + 80*scale)),  
        (int(x - 120*scale), int(y + 120*scale)) # Hạ thấp
    ]
...
#     veheoanimationchapcanhbayxunduoi(screen,400,300,1)
